# ADR-template/0002: Maximal-Explicit Function Attribute Surface
...
judgments static analysis cannot make*, the house uses a **PowerShell function design review checklist** that records the relevant tradeoffs per unit. Its recommendations are consumed through the existing review loop (implement/challenge → audit → ratify). Mechanically
...
4. **Judgment needs review, not just a linter** — capture the reasoning clearly enough to be challenged, then ratify through review.
...
1. **Maximal-explicit attribute surface + a design review checklist + custom PSScriptAnalyzer rules for the mechanical subset** (chosen).
...
- **The review checklist.** A **PowerShell function design review checklist** produces, per unit, an advisory report answering in
...
- **Consumption is adversarial.** Recommendations flow through the existing review loop: implement/challenge, audit, and owner ratification.
...
- **Evidence.** A benchmark harness settles construct/perf determinations empirically — metric = dispatch
...
- The checklist is a review aid that must be challenged, not trusted blindly.
...
2. A documented review checklist plus adversarial human review is more scalable and consistent than ad hoc per-function judgment.
